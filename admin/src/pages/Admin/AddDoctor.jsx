...
const AddDoctor = () => {
  ...
  const{data}=await axios.post(backendUrl+'/api/admin/add-doctor',formData,{headers:{atoken: aToken}})
      ...
      // Axios errors have useful fields: response (server response), request (sent request)
      if (error.response) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
       ...
       <button type="submit" className='rounded-full bg-[#5f6FFF] px-10 py-3 mt-4 text-white cursor-pointer'>Add Doctor</button>
      </div>
    </form>
  )
}
